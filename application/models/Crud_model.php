   ...
   function get_product_price($product_id)
    {
        ...
        $category_id = $this->get_type_name_by_id('product', $product_id, 'category');
        $gst_amount  = $this->get_type_name_by_id('category', $category_id, 'gst_amount');
        ...
        $return_amount =  number_format((float) $number, 2, '.', '');
        return $return_amount;
        //return get_include_price($return_amount,$tax,$tax_type);
    }
    
     function get_product_price_with_gst($product_id,$price_type=null)
    {
        
        if($price_type==1){
            $price         = $this->get_type_name_by_id('product', $product_id, 'sale_price');
        }else if($price_type==2){
            $price         = $this->get_type_name_by_id('product', $product_id, 'purchase_price');
        }else{
            $price         = $this->get_type_name_by_id('product', $product_id, 'purchase_price');
        }
        ...
        $get_gst_amount = ($number*$gst_amount/100);
        $include_gst_price = ($number+$get_gst_amount);    
                               
        $return_amount =  number_format((float) $include_gst_price, 2, '.', '');
        return currency($return_amount);
    }
    
    function cart_get_price_gst($product_id){
        $price         = $this->get_type_name_by_id('product', $product_id, 'purchase_price');
        $discount      = $this->get_type_name_by_id('product', $product_id, 'discount');
        $discount_type = $this->get_type_name_by_id('product', $product_id, 'discount_type');
	...
	function get_brands($type,$id,$vendor=''){
		...
		foreach($sub_cats as $row){
			$n_brands = json_decode($this->db->get_where('subsubcat',array('id'=>$row['id']))->row()->brand);
    ...
    function sub_details_by_cat($cat=''){
        $subs = $this->db->get_where('subsubcat',array('category_id'=>$cat))->result_array();
        $result = array();
        foreach($subs as $row){
            $result[] = array(
                            'subsub_id' => $row['id'],
                            'sub_id' => $row['subsubcat_id'],
                            'subsub_name' => str_replace("'",' ',$row['subsubcat_name']),
                            'min' => round($this->crud_model->get_range_lvl('sub_category', $row['subsubcat_id'], "min")),
                            'max' => round($this->crud_model->get_range_lvl('sub_category', $row['subsubcat_id'], "max")),
                            'brands' => str_replace("'",' ',join(';;;;;;',$this->get_brands('subsubcat',$row['id'])))
                        );
        }
	...
	function if_publishable_subcategory($id){
        ...
        function if_publishable_subsubcategory($subid){
        $sub_category_data = $this->db->get_where('subsubcat',array('sub_id'=>$subid));
        $physical_product_activation = $this->db->get_where('general_settings',array('type'=>'physical_product_activation'))->row()->value;
        $digital_product_activation = $this->db->get_where('general_settings',array('type'=>'digital_product_activation'))->row()->value;
