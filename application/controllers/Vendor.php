           ...
           while($data = fgetcsv($handle))
           {
            ...
            if($iss==1){
              ...
              if($this->input->post('upload_type')==2){
                ...
                $p_name             = $data[3]; 
                $p_sale_price       = $data[4];
                $p_purchase_price   = $data[5];
                $p_description      = $data[6];
                $p_unit             = $data[7];
                $p_tag              = $data[8];
                $p_shipping_cost    = $data[9];
                $p_discount         = $data[10];
                $p_discount_type    = $data[11];
                $p_img              = $data[12];
             ...
             if(!empty($p_category) and !empty($p_category) and !empty($p_sub_category) and !empty($p_sub_category) and !empty($p_brand) and !empty($p_brand)){
               ...
               $select_category = mysqli_query($conn,"select * from `category` where category_name='".$p_category."' ");

               $res_category = mysqli_fetch_assoc($select_category);

               
               $select_subcategory = mysqli_query($conn,"select * from `sub_category` where sub_category_name='".$p_sub_category."' ");
               $res_subcategory = mysqli_fetch_assoc($select_subcategory);

               $select_brand = mysqli_query($conn,"select * from `brand` where name='".$p_brand."' ");
               $res_brand = mysqli_fetch_assoc($select_brand);
               
               $col_val = array(
                'category'=>$res_category['category_id'],
                'sub_category'=>$res_subcategory['sub_category_id'],
                'brand'=>$res_brand['brand_id'],
                'title'=>$p_name,
                ...
                'status'=>'ok'
                      			     ...
                      			     //Add Product 
               $this->db->insert('product', $col_val);
               ...
               if(is_array($product_image_array)){
                ...
                foreach($product_image_array as $img_url){
                  ...
                  $upload_path = "uploads/product_image/product_".$p_id."_".$ib.".jpg";
                  file_put_contents($upload_path, file_get_contents($img_url));
                  $upload_paths = "uploads/product_image/product_".$p_id."_".$ib."_thumb".".jpg";
    ...
    foreach ($products as $row) {
      ...
      $res['title']  = $row['title'];
                ...
                */
                ...
                if($row['status'] == 'ok'){
                  $res['publish']  = '<a href="javascript:;" class="btn btn-success btn-xs ">Public</a>';
                } else {
                  $res['publish']  = '<a href="javascript:;" class="btn btn-danger btn-xs ">Pending</a>';
                }
            ...
            //var_dump($data2['response']);
              ...
              function manage_vendor($para1 = "")
              {
                ...
                //'email' => $this->input->post('email'),
                    ...
                    'gst_number' => $this->input->post('vendor_gst'),
                    'telephone_number' => $this->input->post('vendor_telephone'),
                    'billing_address' => $this->input->post('vendor_billing_address'),
