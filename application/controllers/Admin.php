...
function pricing($para1 = '', $para2 = '')
{
    if (!$this->crud_model->admin_permission('pricing')) {
        redirect(base_url() . 'admin');
    }
    if ($this->crud_model->get_type_name_by_id('general_settings','68','value') !== 'ok') {
        redirect(base_url() . 'admin');
    ...
    if ($para1 == 'do_add') {
        $data['category_name'] = $this->input->post('category_name');
        $data['parking_charge'] = $this->input->post('parking_charge');
        $data['status'] = $this->input->post('status');
        $data['created_date'] = date('Y-m-d');
        $this->db->insert('price', $data);
    } else if ($para1 == 'edit') {
        $page_data['pricing_data'] = $this->db->get_where('price', array(
            'id' => $para2
        ))->result_array();
        $this->load->view('back/admin/pricing_edit', $page_data);
    } elseif ($para1 == "update") {
        $data['category_name'] = $this->input->post('category_name');
        $data['parking_charge'] = $this->input->post('parking_charge');
        $data['status'] = $this->input->post('status');
        ...
        $page_data['all_pricing'] = $this->db->get('price')->result_array();
        $this->load->view('back/admin/pricing_list', $page_data);
    } elseif ($para1 == 'add') {
        $this->load->view('back/admin/pricing_add');
    } else {
        $page_data['page_name']      = "pricing";
        $page_data['all_categories'] = $this->db->get('price')->result_array();
        $this->load->view('back/index', $page_data);
    }
}
...
/*Product blog_category add, edit, view, delete */
// function image_upload($para1 = '', $para2 = '')
// {
//     if (!$this->crud_model->admin_permission('blog')) {
//         redirect(base_url() . 'admin');
//     }
...
//             'blog_category_id' => $para2
//         ))->result_array();
//         $this->load->view('back/admin/image_category_edit', $page_data);
//     } elseif ($para1 == "update") {
//         $data['name'] = $this->input->post('name');
//         $this->db->where('blog_category_id', $para2);
//         $this->db->update('blog_category', $data);
...
//         $this->db->order_by('blog_category_id', 'desc');
//         $page_data['all_categories'] = $this->db->get('blog_category')->result_array();
//         $this->load->view('back/admin/image_category_list', $page_data);
//     } elseif ($para1 == 'add') {
//         $this->load->view('back/admin/image_category_add');
//     } else {
//         $page_data['page_name']      = "image_category";
//         $page_data['all_categories'] = $this->db->get('blog_category')->result_array();
//         $this->load->view('back/index', $page_data);
//     }
// }
...
function product($para1 = '', $para2 = '', $para3 = '')
{
    ...
    if ($para1 == 'do_add') {
        ...
        $data['sub_sub_category_id']= $this->input->post('sub');
...
//add html for action
        ...
        echo $this->crud_model->select_html('sub_category', 'sub_category', 'sub_category_name', 'add', 'demo-chosen-select required', '', 'category', $para2, 'get_subsub');
    } elseif ($para1 == 'subsubcat_by_subcat') {
        echo $this->crud_model->select_html('subsubcat', 'sub', 'subsubcat_name', 'add', 'demo-chosen-select required', '', 'subsubcat_id', $para2, 'get_brnd');
    }elseif ($para1 == 'brand_by_sub') {
       $brands=json_decode($this->crud_model->get_type_name_by_id('subsubcat',$para2,'brand'),true);
       if(empty($brands)){
        echo translate("No brands are available for this sub sub category");
    } else {
...
//bulk upload product
function product_bulk_upload($cat_id=null,$sub_cat_id=null){
    if($this->input->post('csv_upload')){
            ...
            if(strtolower(end($filename)) == "csv"){
                ...
                while($data = fgetcsv($handle))
                {
                    ...
                    if($iss==1){
                        ...
                        $added_by = json_encode(array('type'=>'admin','id'=>$this->session->userdata('admin_id')));
//Product Upload Type
                        if($this->input->post('upload_type')==2){
//Product Category get in input box
                            $p_category         = $this->input->post('category');
                            $p_sub_category     = $this->input->post('sub_category');
                            $p_brand            = $this->input->post('brand');
                            $p_name             = $data[0]; 
                            $p_sale_price       = $data[1];
                            $p_purchase_price   = $data[2];
                            $p_description      = $data[3];
                            $p_unit             = $data[4];
                            $p_tag              = $data[5];
                            $p_shipping_cost    = $data[6];
                            $p_discount         = $data[7];
                            $p_discount_type    = $data[8];
                            $p_img              = $data[9];
                        }else if($this->input->post('upload_type')==1){
//Product Category get in csv file
                            $p_category         = $data[0];
                            $p_sub_category     = $data[1];
                            $p_subsub_category  = $data[2];
                            $p_brand            = $data[3];
                            $p_name             = $data[4]; 
                            $p_sale_price       = $data[5];
                            $p_purchase_price   = $data[6];
                            $p_description      = $data[7];
                            $p_unit             = $data[8];
                            $p_tag              = $data[9];
                            $p_shipping_cost    = $data[10];
                            $p_discount         = $data[11];
                            $p_discount_type    = $data[12];
                            $p_img              = trim($data[13]);
                        }
//convert to array sting one.jpg@two.jpg@three.jpg    array('one.jpg','two.jpg','three.jpg')
                        $product_image_array = explode('@',$p_img);

                        // echo "<pre>";
                        // print_r($product_image_array);
                        // exit();

                        if(empty($p_discount) or !is_numeric($p_discount)){
                            $p_discount = 0;
                        }

                        if(!is_array($p_discount_type,array('percent','amount') ) ){
                            $p_discount_type = 'percent';
                        }
                        if(!empty($p_category) and !empty($p_category) and !empty($p_sub_category) and !empty($p_sub_category) and !empty($p_brand) and !empty($p_brand)){

                            $query = $this->db->get_where('category', array('category_name' =>trim($p_category)));
                            ...
                            $query1 = $this->db->get_where('sub_category', array('sub_category_name' => trim($p_sub_category)));
                            foreach ($query1->result() as $sub_category)
                            {
                                $sub_category->sub_category_id;

                            }

                            $query2 = $this->db->get_where('subsubcat', array('subsubcat_name' => trim($p_subsub_category)));
                            foreach ($query2->result() as $subsub_category)
                            {
                                $subsub_category->sub_id;

                            }

                            $query3 = $this->db->get_where('brand', array('name' => trim($p_brand)));

                            foreach ($query3->result() as $brand)
                            {
                                $brand->brand_id;
                            ...
                            $col_val = array(
                                ...
                                'sub_sub_category_id'=>$subsub_category->sub_id,
                                ...
                                'num_of_imgs'=>count($product_image_array), 
                                'added_by'=> $added_by,
                                'add_timestamp'=>time(),
                                'download'=>NULL,
                                'current_stock'=>0,
...
//Add Product 
                            $test = $this->db->insert('product', $col_val);
                            $p_id = $this->db->insert_id();

                            
//save file from server
                            $ib = 0; 
                            if(is_array($product_image_array)){
//Add with product_270_1.jpg  product_270_1_thumb.jpg 

                                foreach($product_image_array as $img_url){

                                    $ib++;
                                    $ib = $this->crud_model->file_exist_ret("product",$p_id,$ib);
                                    $upload_path = FCPATH."uploads/product_image/product_".$p_id."_".$ib.".jpg";

                                    file_put_contents($upload_path, file_get_contents($img_url));
                                    
                                    $upload_paths = FCPATH."uploads/product_image/product_".$p_id."_".$ib."_thumb".".jpg";
                                    file_put_contents($upload_paths, file_get_contents($img_url));
                                }
                            }
                            $success_upload_file++;
                        }else{
...
//Product Upload Type
                if($this->input->post('upload_type')==1){
                    $f_all_upload_file     = "all_upload_file";
                    $f_success_upload_file = "success_upload_file";
                    $f_error_upload_file   = "error_upload_file";
                }else if($this->input->post('upload_type')==2){ 
                    $f_all_upload_file     = "all_upload_file2";
                    $f_success_upload_file = "success_upload_file2";
                    $f_error_upload_file   = "error_upload_file2";
                }
                $this->session->set_flashdata($f_all_upload_file, $all_upload_file);
                $this->session->set_flashdata($f_success_upload_file, $success_upload_file);
                $this->session->set_flashdata($f_error_upload_file,$error_upload_file);
...
//bulk upload brand
function brand_bulk_upload($cat_id=null,$sub_cat_id=null){
    if($this->input->post('csv_upload')){
        if(!empty($_FILES['product_file']['name'])){
            $filename = explode(".", $_FILES['product_file']['name']);
            if(strtolower(end($filename)) == "csv"){
...
//Add with product_270_1.jpg  product_270_1_thumb.jpg 
                                foreach($product_image_array as $img_url){
                                    $ib++;
                                    $ib = $this->crud_model->file_exist_ret("brand",$p_id,$ib);
                                    $upload_path = "uploads/brand_image/brand_".$p_id."_".$ib.".jpg";
                                    file_put_contents($upload_path, file_get_contents($img_url));
                                    $upload_paths = "uploads/brand_image/brand_".$p_id."_".$ib."_thumb".".jpg";
                                    file_put_contents($upload_paths, file_get_contents($img_url));
                                } 
                            }
                            $success_upload_file++;
...
//Product Upload Type
                ...
                $this->session->set_flashdata($f_success_upload_file, $success_upload_file);
                $this->session->set_flashdata($f_error_upload_file,$error_upload_file);
                redirect('admin/brand_bulk_upload');
            }else{
                $this->session->set_flashdata('message', 'Please Select CSV File only');
                redirect('admin/brand_bulk_upload');
            }
        ...
        $page_data['page_name']   = "brand_bulk_upload";
        $this->load->view('back/index',$page_data);    
    }
}
//bulk upload category
function category_bulk_upload($cat_id=null,$sub_cat_id=null){
    if($this->input->post('csv_upload')){
        if(!empty($_FILES['product_file']['name'])){
            $filename = explode(".", $_FILES['product_file']['name']);
            if(strtolower(end($filename)) == "csv"){
...
//Add Product 
                            $this->db->insert('category', $col_val);
                            $p_id = $this->db->insert_id();
//save file from server
                            $ib = 0; 
                            if(is_array($product_image_array)){
//Add with product_270_1.jpg  product_270_1_thumb.jpg 
                                foreach($product_image_array as $img_url){
                                    $ib++;
                                    $ib = $this->crud_model->file_exist_ret("category",$p_id,$ib);
                                    $upload_path = "uploads/category_image/category_".$p_id."_".$ib.".jpg";
                                    file_put_contents($upload_path, file_get_contents($img_url));
                                    $upload_paths = "uploads/category_image/category_".$p_id."_".$ib."_thumb".".jpg";
                                    file_put_contents($upload_paths, file_get_contents($img_url));
                                } 
                            }
                            $success_upload_file++;
...
//Product Upload Type
                ...
                $this->session->set_flashdata($f_success_upload_file, $success_upload_file);
                $this->session->set_flashdata($f_error_upload_file,$error_upload_file);
                redirect('admin/category_bulk_upload');
            }else{
                $this->session->set_flashdata('message', 'Please Select CSV File only');
                redirect('admin/category_bulk_upload');
            }
        ...
        $page_data['page_name']   = "category_bulk_upload";
        $this->load->view('back/index',$page_data);    
    }
}
//bulk upload subcategory
function sub_category_bulk_upload($cat_id=null,$sub_cat_id=null){
    if($this->input->post('csv_upload')){
        if(!empty($_FILES['product_file']['name'])){
            $filename = explode(".", $_FILES['product_file']['name']);
...
//convert to array sting one.jpg@two.jpg@three.jpg    array('one.jpg','two.jpg','three.jpg')
                        if(!empty($sub_name)){
                            ...
                            $query = $this->db->get_where('category', array('category_name' =>trim($sub_category)));
                            ...
                            // $conn = mysqli_connect("localhost","root","","aromecx");
                            // $select_category = mysqli_query($conn,"select * from `category` where category_name='".trim($sub_category)."' ");
                            // $res_category = mysqli_fetch_assoc($select_category);
...
//Add Product 
                            $this->db->insert('sub_category', $col_val);
                            $p_id = $this->db->insert_id();
//save file from server
                            $ib = 0; 
                            if(is_array($product_image_array)){
//Add with product_270_1.jpg  product_270_1_thumb.jpg 
                                foreach($product_image_array as $img_url){
                                    $ib++;
                                    $ib = $this->crud_model->file_exist_ret("sub_category",$p_id,$ib);
                                    $upload_path = "uploads/sub_category_image/sub_category_".$p_id."_".$ib.".jpg";
                                    file_put_contents($upload_path, file_get_contents($img_url));
                                    $upload_paths = "uploads/sub_category_image/sub_category_".$p_id."_".$ib."_thumb".".jpg";
                                    file_put_contents($upload_paths, file_get_contents($img_url));
                                } 
                            }
                            $success_upload_file++;
...
//Product Upload Type
                ...
                $this->session->set_flashdata($f_success_upload_file, $success_upload_file);
                $this->session->set_flashdata($f_error_upload_file,$error_upload_file);
                redirect('admin/sub_category_bulk_upload');
            }else{
                $this->session->set_flashdata('message', 'Please Select CSV File only');
                redirect('admin/sub_category_bulk_upload');
            }
        ...
        $page_data['page_name']   = "sub_category_bulk_upload";
        $this->load->view('back/index',$page_data);    
    }
}
function product_export($a){
    if($a=="export"){ 
        $delimiter = ",";
        $filename = "products_" . rand() . ".csv";
//create a file pointer
        $f = fopen('php://memory', 'w');
//set column headers
        $fields = array('Category','sub category','Brand','Name', 'Sale Price', 'Purchase Price','Description','unit','Tags sepreated by a,b,c','Shipping Cost','Stock',
            'discount','discount type (percent or amount)','Multi Image sepreated by a@b@c');
        fputcsv($f, $fields, $delimiter); 
        $productResult = $this->db->get('product')->result_array();
        foreach($productResult as $pr) { 
            $num_of_imgs = $pr['num_of_imgs'];
            $ib = 0; 
            $upload_path = "";
            for($ib; $ib <= $num_of_imgs; $ib++){
                $upload_path .= base_url()."uploads/product_image/product_".$pr['product_id']."_".$ib.".jpg@";
            }
            $conn = mysqli_connect("localhost","root","","aromecx");
            $select  = mysqli_query($conn,"select * from `category` where category_id='".$pr['category']."' ");
            $res_category = mysqli_fetch_assoc($select);
            $select_subcategory  = mysqli_query($conn,"select * from `sub_category` where sub_category_id='".$pr['sub_category']."' ");
            $res_subcategory = mysqli_fetch_assoc($select_subcategory);
            $brand  = mysqli_query($conn,"select * from `brand` where brand_id='".$pr['brand']."' ");
            $brand = mysqli_fetch_assoc($brand);
            $total_stock =$pr['current_stock']+$pr['unit'];
            $lineData = array($res_category['category_name'],$res_subcategory['sub_category_name'],$brand['name'],$pr['title'],$pr['sale_price'], $pr['purchase_price'],$pr['description'],
                $pr['unit'],$pr['tag'],$pr['shipping_cost'],$total_stock,$pr['discount'],$pr['discount_type'],rtrim($upload_path,"@"));
...
function show_c_s_b_id(){
    $page_data['all_categories'] = $this->db->get('category')->result_array();
    $page_data['page_name']   = "show_c_s_b_id";
    $this->load->view('back/index',$page_data);    
}
...
function package_payment($para1 = '', $para2 = '', $para3 = '')
{
    ...
    if ($para1 == 'view') {
        ...
        foreach ($products as $row) {
            ...
            $res['payment_type'] = "
            <center><span class='badge badge-primary'>".$row['payment_type']."</span></center>
            ";
            ...
            if ($row['payment_status'] == 'paid') {
                $res['status'] = "
                <center><span class='badge badge-success'>".translate($row['payment_status'])."</span></center>
                ";
            } elseif ($row['payment_status'] == 'due') {
                $res['status'] = "
                <center><span class='badge badge-danger'>".translate($row['payment_status'])."</span></center>
                ";
            } elseif ($row['payment_status'] == 'pending') {
                $res['status'] = "
                <center><span class='badge badge-info'>".translate($row['payment_status'])."</span></center>
                ";
            }
            if($row['status'] == 'ok'){
                $res['customer_status']  = ' <label class="label label-success publish_btn">'.translate('Published').'</label>';
...
/*if(empty($brands)){
echo translate("
<p class='control-label'>No brands are available for this sub category</p>
");
} else {*/
...
// }
    ...
    if ($para2 == 'none') {
        ...
        if(empty($prod_ids)){
            echo translate("
                <p class='control-label'>No Products are available for this brand</p>
                ");
        } else {
...
function sales($para1 = '', $para2 = '')
{
    ...
    if ($para1 == 'delete') {
        ...
        $this->db->update('sale', $data);
//Send sms on mobile
        $info = json_decode($this->db->get_where('sale', array(
            'sale_id' => $para2
        ))->row()->shipping_address,true);
        send_sms($info['phone'],"Thank you for register with us");
    } elseif ($para1 == 'add') {
...
//}
        ...
        foreach ($words as $row) {
            ...
            $res['no']  = $row['word_id'];
            $res['word']  = '
            <div class="col-md-12 abv">'.ucwords(str_replace('_', ' ', $row['word'])).'</div>
            ';
...
/* shivam add, edit, view, delete, stock increase, decrease, discount */
function shivam($para1 = '', $para2 = '', $para3 = '')
{
    if (!$this->crud_model->admin_permission('product')) {
        redirect(base_url() . 'admin');
    }
    ...
    if ($para1 == 'do_add') {
        ...
        $this->load->view('back/admin/shivam_edit', $page_data);
    } else if ($para1 == 'view') {
        $page_data['product_data'] = $this->db->get_where('product', array(
            'product_id' => $para2
        ))->result_array();
        ...
        $this->db->where('download=',NULL);
        $page_data['all_product'] = $this->db->get('product')->result_array();
        $this->load->view('back/admin/shivam_list', $page_data);
    } elseif ($para1 == 'list_data') {
        $limit      = $this->input->get('limit');
        $search     = $this->input->get('search');
        $order      = $this->input->get('order');
...
//add html for action
            $res['options'] = "  
            <a class=\"btn btn-success btn-xs btn-labeled fa fa-wrench\" data-toggle=\"tooltip\" 
            onclick=\"ajax_set_full('edit','".translate('edit_product')."','".translate('successfully_edited!')."','shivam_edit','".$row['product_id']."');proceed('to_list');\" data-original-title=\"Edit\" data-container=\"body\">
            ".translate('edit')."
            </a>
            <a onclick=\"delete_confirm('".$row['product_id']."','".translate('really_want_to_delete_this?')."')\" 
            class=\"btn btn-danger btn-xs btn-labeled fa fa-trash\" data-toggle=\"tooltip\" data-original-title=\"Delete\" data-container=\"body\">
        ...
        $this->load->view('back/admin/shivam_add');
    } elseif ($para1 == 'add_stock') {
        $data['product'] = $para2;
        $this->load->view('back/admin/product_stock_add', $data);
    } elseif ($para1 == 'destroy_stock') {
        ...
        $page_data['page_name']   = "shivam";
        $page_data['all_product'] = $this->db->get('product')->result_array();
        $this->load->view('back/index', $page_data);
    }
}
/*Product Brand add, edit, view, delete */
function subsubcat($para1 = '', $para2 = '')
{
    if (!$this->crud_model->admin_permission('subsubcat')) {
        redirect(base_url() . 'admin');
    }
    if ($this->crud_model->get_type_name_by_id('general_settings','68','value') !== 'ok') {
        redirect(base_url() . 'admin');
    ...
    if ($para1 == 'do_add') {
        ...
        $data['subsubcat_id']        = $this->input->post('sub_category');
        $data['subsubcat_name']        = $this->input->post('name');
        if($this->input->post('brand')==NULL)
        {
            $data['brand']             = '[]';
        }
        ...
        $this->db->where('brand_id', $id);
        $this->db->update('brand', $data_banner);
        $set = $this->crud_model->set_category_data(0);
        recache();
    } elseif ($para1 == "update") {
        $type                = 'subsubcat';
        $data['category_id']        = $this->input->post('category');
        ...
        $getid = $this->db->where('sub_id', $para2);
        $this->db->update('subsubcat', $data);
        if($_FILES['img']['name']!== ''){
            $path = $_FILES['img']['name'];
            $ext = pathinfo($path, PATHINFO_EXTENSION);
            $data_logo['logo']       = 'brand_'.$para2.'.'.$ext;
            $this->crud_model->file_up("img", "subsubcat", $para2, '', 'no', '.'.$ext);
            $this->db->where('id', $para2);
            $this->db->update('subsubcat', $data_logo);
        }
        $this->crud_model->set_category_data(0);
        recache();
    } elseif ($para1 == 'delete') {
        unlink("uploads/brand_image/" .$this->crud_model->get_type_name_by_id('brand',$para2,'logo'));
        $this->db->where('sub_id', $para2);
        $this->db->delete('subsubcat');
        $this->crud_model->set_category_data(0);
        recache();
    } elseif ($para1 == 'multi_delete') {
        $ids = explode('-', $param2);
        ...
        $page_data['subsubcat_data'] = $this->db->get_where('subsubcat', array(
            'sub_id' => $para2
        ))->result_array();
        $this->load->view('back/admin/subsubcat_edit', $page_data);
    } elseif ($para1 == 'list') {
        $this->db->order_by('sub_id', 'desc');
        $page_data['all_subsubcat'] = $this->db->get('subsubcat')->result_array();
        $this->load->view('back/admin/subsubcat_list', $page_data);
    } elseif ($para1 == 'add') {
        $this->load->view('back/admin/subsubcat_add');
    } else {
        $page_data['page_name']  = "subsubcat";
        $page_data['all_brands'] = $this->db->get('brand')->result_array();
        $this->load->view('back/index', $page_data);
    }
}
...
//bulk upload category
function subsubcategory_bulk_upload($cat_id=null,$sub_cat_id=null){
    if($this->input->post('csv_upload')){
        if(!empty($_FILES['product_file']['name'])){
            $filename = explode(".", $_FILES['product_file']['name']);
            if(strtolower(end($filename)) == "csv"){
...
//convert to array sting one.jpg@two.jpg@three.jpg    array('one.jpg','two.jpg','three.jpg')
                        if(!empty($c_name)){
                            ...
                            $query = $this->db->get_where('category', array('category_name' => trim($c_name)));
                            foreach ($query->result() as $category)
                            {
                                $category->category_id;
                            ...
                            $query1 = $this->db->get_where('sub_category', array('sub_category_name' => trim($c_subname)));
                            foreach ($query1->result() as $subcategory)
                            {
                            ...
                            $col_val = array(
                                ...
                                'subsubcat_id'=>$subcategory->sub_category_id,
                                'subsubcat_name'=>$c_subsubname,
                                'brand'=>$brand,
                            ); 
//Add Product 
                            $this->db->insert('subsubcat', $col_val);
                            $p_id = $this->db->insert_id();
//save file from server
                            $success_upload_file++;
                        }else{
...
//Product Upload Type
                ...
                $this->session->set_flashdata($f_success_upload_file, $success_upload_file);
                $this->session->set_flashdata($f_error_upload_file,$error_upload_file);
                redirect('admin/subsubcategory_bulk_upload');
            }else{
                $this->session->set_flashdata('message', 'Please Select CSV File only');
                redirect('admin/subsubcategory_bulk_upload');
            }
        ...
        $page_data['page_name']   = "subsubcategory_bulk_upload";
        $this->load->view('back/index',$page_data);    
    }
}
