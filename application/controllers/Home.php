...
class Home extends CI_Controller
{
    ...
    function category($para1 = "", $para2 = "", $min = "", $max = "", $text ='')
    {
        ...
        $page_data['cat_gst_amount']   = $page_data['category_data'][0]['gst_amount'];
   ...
   function vendor_logup($para1 = "", $para2 = "")
                ...
                if($safe == 'yes'){
                    if($this->crud_model->get_settings_value('general_settings','captcha_status','value') == 'ok'){
                        ...
                        if ($response['success']) {
                            ...
                            //Extra add new filed
                        $data['gst_number'] 	  =  $this->input->post('vendor_gst');
                        $data['phone']     =  $this->input->post('vendor_phone');
                        $data['telephone_number'] =  $this->input->post('vendor_telephone');
                        $data['billing_address']  =  $this->input->post('vendor_billing_address');
    ...
    function cart($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        if ($para1 == "add") {
            ...
            $data = array(
                ...
                'price' =>$this->crud_model->cart_get_price_gst($para2),
    ...
    /* FUNCTION: CCavanue : ccavRequestHandler*/
    function ccavRequestHandler(){ 
        $this->load->view('front/ccavenue/ccavRequestHandler');

    } 

   function ccavresponseHandler(){
         
        $this->load->view('front/ccavenue/Crypto');
        
        error_reporting(0);
	
    	$workingKey  = '5D02BFBC493C2A594425A50C0B791518';		//Working Key should be provided here.
      ...
      //$this->load->view('front/ccavenue/ccavResponseHandler');
   }
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
        ...
        if ($this->input->post('payment_type') == 'ccavenue') {
            if ($para1 == 'go') {
                ...
                /*
                $working_key   = $this->crud_model->get_settings_value('business_settings', 'ccav_working_key', 'value');
                $access_code   = $this->crud_model->get_settings_value('business_settings', 'ccav_access_code', 'value');
                $merchant_data = $this->crud_model->get_settings_value('business_settings', 'ccav_merchant_data', 'value');
                */
                ...
                //Send data from ccavenue
                $page_data['working_key']       = $working_key; 
                $page_data['access_code']       = $access_code; 
                $page_data['merchant_data']     = $merchant_data;
                
                $page_data['firstname']         = $this->db->get_where('user', array('user_id' => $user_id))->row()->username;
                $page_data['email']             = $this->db->get_where('user', array('user_id' => $user_id))->row()->email;
                $page_data['phone']             = $this->db->get_where('user', array('user_id' => $user_id))->row()->phone;
                $page_data['address1']          = $this->db->get_where('user', array('user_id' => $user_id))->row()->address1;
                $page_data['city']              = $this->db->get_where('user', array('user_id' => $user_id))->row()->city;
                $page_data['state']             = $this->db->get_where('user', array('user_id' => $user_id))->row()->state;
                $page_data['country']           = $this->db->get_where('user', array('user_id' => $user_id))->row()->country;
                $page_data['zip']               = $this->db->get_where('user', array('user_id' => $user_id))->row()->zip;
               
                
                
                $page_data['sale_code']         = $sale_code;
                $page_data['sale_id']           = $sale_id;
                $page_data['amount']            = $grand_total;
                $page_data['del_address']       = json_encode($_POST); 
                
                
                $this->load->view('front/ccavenue/index', $page_data);
